...
- `main` 是主分支；当前工作分支代码基线为 `v0.6 Review Audit + Catalyst Watch Phase 3`。
...
```json
{"status":"ok","version":"0.6.0","database":"sqlite","connectors":["cninfo"],"market_provider":"tencent","monitor_findings":true,"mode":"local-first"}
```
...
| `混合模式 v0.6.0` | FastAPI 健康检查通过 | 可使用自动发现、手动行情刷新、透明 finding 和审核历史 |
...
1. 打开 `/api/health`，确认 `status` 为 `ok`、版本为 `0.6.0`、`market_provider` 为 `tencent`，且 `monitor_findings=true`。
...
flowchart LR
    ...
    W -->|"同源 /api"| A["FastAPI"]
    A --> D["SQLite: watchlist / runs / snapshots / findings / evidence / review_history / scores"]
...
3. Catalyst Watch Phase 3 仍只有腾讯单一行情 provider，定位为本地低频、best-effort 数据；尚无 fallback、熔断、重试、交易日判断、运行锁或调度器。
...
`Catalyst Watch Phase 4: 本地定时任务与数据源韧性`：

1. 增加交易日和交易时段判断，默认低频运行，不伪装成逐笔实时行情。
2. 增加单实例运行锁、幂等、去重、有界重试和单项失败隔离。
3. 为 provider 增加 fallback、熔断、last-good cache、结构化错误和 Doctor 页面。
4. 为每次运行记录 trace ID、触发方式、数据源决策、重试与恢复过程。
5. 本阶段仍不发送买卖建议，不引入通知、交易动作或 LLM。
