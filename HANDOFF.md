...
静态模式下“自动发现”按钮必须是禁用状态，这是运行边界，不是股票代码或页面故障。
...
1. 打开 `/api/health`，确认 `status` 为 `ok`。
...
API 单元测试使用临时 SQLite 和假连接器，不依赖外网。修改 CNINFO 连接器时，除了单元测试，还应做一次真实公告冒烟测试，但不要把外网测试放进必过 CI。
...
3. 接入合规、稳定的行情与板块数据，自动补充事件前后涨跌、成交量、换手、相对板块和同行表现。
